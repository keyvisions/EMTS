...
The projection postulate—observable reality is $\Re(z) = \projReal$ (Section~\ref{sec:projection-principle} of Chapter~\ref{ch:emts-framework})—has one consequence whose full implications this chapter develops: the projection is \textbf{many-to-one}. An infinite number of complex points
\[
...
The quantization arises from the periodic structure in $\theta$. Physical states must satisfy the single-valuedness condition (cf.\ ``Quantization from closed-orbit conditions'' in Chapter~\ref{ch:quantum-properties})
\[
